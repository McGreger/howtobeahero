...
export default class SystemDataModel extends foundry.abstract.TypeDataModel {
   ...
   * @property {typeof DataModel} [systemFlagsModel]  Model that represents flags data within the how-to-be-a-hero namespace.
   */
  ...
  static defineSchema() {
    ...
    for ( const template of this._schemaTemplates ) {
      if ( !template.defineSchema ) {
        throw new Error(`Invalid how-to-be-a-hero template mixin ${template} defined on class ${this.constructor}`);
      }
  ...
  async _preCreate(data, options, user) {
    ...
    if ( actor.itemTypes[data.type]?.length ) {
      ui.notifications.error(game.i18n.format("HTBAH.ActorWarningSingleton", {
...
/**
 * Abstract base class for HTBAH system-specific Document data models
 */
export class AbstractHowToBeAHeroDataModel extends SystemDataModel {
  /** @override */
  static defineSchema() {
  ...
  /**
   * Provide a rich tooltip for this document
   * @returns {Promise<{content: string, classes: string[]}>}
   */
  async richTooltip() {
    // Default implementation
    return {
      content: `<h3>${this.parent.name}</h3><p>${this.description}</p>`,
      classes: ['htbah-tooltip']
    };
  }
}

/**
 * Abstract base class for HTBAH Actor data models
 */
export class AbstractHowToBeAHeroActorDataModel extends AbstractHowToBeAHeroDataModel {
  /** @override */
  static defineSchema() {
    const fields = foundry.data.fields;
    return foundry.utils.mergeObject(super.defineSchema(), {
      // Add actor-specific fields here
    });
  }
...
/**
 * Abstract base class for HTBAH Item data models
 */
export class AbstractHowToBeAHeroItemDataModel extends AbstractHowToBeAHeroDataModel {
  /** @override */
  static defineSchema() {
    const fields = foundry.data.fields;
    return foundry.utils.mergeObject(super.defineSchema(), {
