...
export default class ItemListControlsElement extends HTMLElement {
  ...
  connectedCallback() {
     ...
     */
    get prefs() {
      return game.user.getFlag("how-to-be-a-hero", `sheetPrefs.${this.app.object.type}.tabs.${this.tab}`);
    }
    ...
    #buildHTML() {
      ...
      search.innerHTML = `
        ...
        <ul class="unlist controls">
          <li>
            <button type="button" class="unbutton filter-control" data-action="clear" data-tooltip="HTBAH.FilterClear"
                    aria-label="${game.i18n.localize("HTBAH.FilterClear")}">
      ...
      if ( options.length ) {
        ...
        item.innerHTML = `
          <button type="button" class="unbutton filter-control filter" data-action="filter"
                  aria-label="${game.i18n.localize("HTBAH.Filter")}">
    ...
    _applyFilters() {
      ...
      let entries = this.#getFilteredItems(properties);
      if ( !entries ) return;
    ...
    /**
     * Get filtered items based on properties.
     * @param {Set} properties
     * @returns {Array}
     * @private
     */
    #getFilteredItems(properties) {
      const items = this.app.object.items.contents;
      if (!properties.size) return items;
      return items.filter(item => {
        // Implement your filtering logic here
        // For example:
        if (properties.has("equipped") && !item.system.equipped) return false;
    ...
    _applyGrouping() {
      if (this._controls.group) {
        ...
        const sections = {};
        const sectionElements = this.list.querySelectorAll(".items-section");
        
        // If there are no section elements, we can't group
        if (sectionElements.length === 0) {
          console.warn("No .items-section elements found. Grouping cannot be applied.");
          return;
        }
        ...
        sectionElements.forEach(section => {
          ...
          if (type) {
            ...
            if (itemList) {
              ...
              console.warn(`No .item-list found in section with type ${type}`);
            }
          } else {
            console.warn("Found .items-section without a data-type attribute");
          }
        });
        ...
        items.forEach(item => {
          ...
          const sectionType = group ? grouped : ungrouped;
          const targetSection = sections[sectionType];
          
          if (targetSection) {
            targetSection.appendChild(item);
          } else {
            console.warn(`No section found for item type: ${sectionType}`);
          }
        });
      }
    ...
    async _onToggleMode(event) {
      ...
      const current = game.user.getFlag("how-to-be-a-hero", flag);
      let value;
      if ( action === "group" ) value = current === false;
      else if ( action === "sort" ) value = current === "a" ? "m" : "a";
      await game.user.setFlag("how-to-be-a-hero", flag, value);
      if ( action === "group" ) {
        this._initGrouping();
        this._applyGrouping();
      } else if ( action === "sort" ) {
